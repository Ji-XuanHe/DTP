...
class FrequencyStructuralDecoupling(nn.Module):
    """Paper name: Frequency-aware Structural Decoupling (FSD)."""
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        texture_llr = torch.stack([separated[:, index, :, :] for index in range(3)], dim=1)
        luminance_llr = torch.stack([separated[:, index, :, :] for index in range(3, 6)], dim=1)
        return luminance_llr, texture_llr


class DecomposeNet(FrequencyStructuralDecoupling):
    """Compatibility alias using the original release order: texture first, luminance second."""

    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        luminance_llr, texture_llr = super().forward(x)
        return texture_llr, luminance_llr


FSDModule = FrequencyStructuralDecoupling
